...
function availableScales(scales, width, height) {
    ...
    for (var i = 0; i < scales.length; i++) {
        ...
        if (!existing || Math.abs(requested - effective) < existing.distance)
            byEffectiveScale[key] = { value: String(scales[i]), index: i, distance: Math.abs(requested - effective) }
    }
    return Object.keys(byEffectiveScale).map(function(key) { return byEffectiveScale[key] })
...
function matchingScaleIndex(scales, currentScale, width, height) {
    ...
    for (var i = 0; i < scales.length; i++) {
        ...
        var d = Math.abs(Number(scales[i]) - current)
...
function parseDisplays(raw) {
    var displays = []
    try { displays = raw ? JSON.parse(String(raw)) : [] } catch (e) { displays = [] }
    if (!Array.isArray(displays)) displays = []
    var count = 0
    for (var i = 0; i < displays.length; i++) if (displays[i] && displays[i].enabled) count++
    return { displays: displays, enabledDisplayCount: count }
}
