...
function projectifier.discover_projects()
	...
	if not success then
		wezterm.log_error("Cannot read projects directory: " .. projectifier.projects_dir .. " - " .. (err or "unknown error"))
		return projects
	end
...
function projectifier.show_project_picker(window, pane)
	...
	if #projects == 0 then
		window:toast_notification("WezTerm", "No project configurations found in " .. projectifier.projects_dir, nil, 3000)
		return
	end
